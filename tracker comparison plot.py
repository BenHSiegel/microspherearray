...
fig, ax = plt.subplots()
ax.semilogy(f_trackpy[:-1], ASDx_trackpy[:-1],label='TrackPy')
ax.semilogy(fx_eigen[:-1], ASDx_eigen[:-1],label='Eigenframe')
ax.semilogy(fx_cnn[:-1], ASDx_cnn[:-1],label='CNN')

ax.legend()
ax.set_xlabel('Frequency (Hz)')
ax.set_ylabel(r'Amplitude Spectral Density ($m/ \sqrt{Hz}$)')

fig1, ax1 = plt.subplots()

ax1.loglog(f_trackpy[:-1], ASDx_trackpy[:-1],label='TrackPy')
ax1.loglog(fx_eigen[:-1], ASDx_eigen[:-1],label='Eigenframe')
ax1.loglog(fx_cnn[:-1], ASDx_cnn[:-1],label='CNN')
ax1.set_xlim([10,500])
ax1.legend(fontsize=16,loc='lower left')
ax1.set_xlabel('Frequency (Hz)',fontsize=18)
ax1.set_ylabel(r'Amplitude Spectral Density ($m/ \sqrt{Hz}$)',fontsize=18)
